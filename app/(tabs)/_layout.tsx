...
import { captureMultipleFromGallery, type CapturedPhotoSet } from '@/lib/capture';
...
function VintageTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const handleCenterScan = () => {
    // Show Take Photo / Choose from Library options
    Alert.alert('Scan Item', 'How would you like to add photos?', [
      {
        text: 'Take Photo',
        onPress: () => {
          console.log('[home] Scan Item pressed -> opening custom camera');
          // Navigate to custom camera screen — useFocusEffect on home picks up result
          navigation.navigate('index');
          // Small delay so home is focused before camera screen pushes on top
          setTimeout(() => {
      ...
      },
      {
        ...
        onPress: async () => {
          ...
          console.log('[home] gallery selected photo count:', photos.length);
          const set: CapturedPhotoSet = {
            front:   photos[0],
            back:    photos[1],
            tag:     photos[2],
            primary: photos[0],
          };
          // Store the full set — useFocusEffect in index.tsx picks it up on focus
          setPendingCaptureSet(set);
          navigation.navigate('index');
        },
      },

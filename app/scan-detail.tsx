...
export default function ScanDetailScreen() {
  ...
  // ── Derived ─────────────────────────────────────────────────────────────────
  // Live rating, recomputed from the CURRENT thrift price. Reading the stored
  // recommendation froze the badge at scan-time: raise the price until the item
  // loses money and it would still read BUY.
  const rating  = normalizeBuyRating(
    calc?.recommendation?.label ?? flip.recommendation?.label ?? flip.buyLabel ?? 'SKIP',
  );
  ...
  const handleSaveThrift = () => {
    ...
    applyUpdate({
      ...
      // System B is what every screen displays. Persisting only buyLabel meant
      // the saved rating never moved, so history and Top Flips kept showing the
      // scan-time verdict after the price changed.
  ...
  return (
    <View style={s.root}>
      ...
      >
        ...
        <View style={s.heroCard}>
          ...
          <View style={s.heroBody}>
            <View style={s.idTitleWrap}>
              ...
              {/* Coach mark moved OUT of this card — see below. heroCard has
                  overflow:'hidden' for its rounded image corners, which clipped
                  the tip in half. */}
            </View>
        ...
        {/* ── Deep Analysis coach mark ────────────────────────────────────
            Sits OUTSIDE heroCard on purpose: that card has overflow:'hidden'
            for its rounded image corners, which clipped this in half. As its
            own full-width card nothing can crop it, and it reads as an
            invitation rather than a stray tooltip. */}
        {showDeepTip && (
          <Pressable
            onPress={handleOpenDeepAnalysis}
            style={({ pressed }) => [s.deepTip, pressed && { opacity: 0.85 }]}
          >
            <View style={s.deepTipArrow} />
            <View style={s.deepTipRow}>
              <View style={s.deepTipIcon}>
                <MaterialIcons name="auto-awesome" size={16} color={GOLD} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={s.deepTipTitle}>See the full breakdown</Text>
                <Text style={s.deepTipText}>
                  Price reasoning, condition, era evidence, risks and where to sell.
                </Text>
              </View>
              <Pressable
                onPress={(e) => { e.stopPropagation?.(); setShowDeepTip(false); }}
                hitSlop={10}
                style={s.deepTipClose}
              >
                <MaterialIcons name="close" size={15} color={MUTED} />
              </Pressable>
            </View>
            <View style={s.deepTipCta}>
              <Text style={s.deepTipCtaText}>Open Deep Analysis</Text>
              <MaterialIcons name="arrow-forward" size={14} color={CREAM} />
            </View>
          </Pressable>
        )}
...
const s = StyleSheet.create({
  ...
  deepTipArrow: {
    position: 'absolute', top: -6.5, left: 26, width: 12, height: 12,
    backgroundColor: '#FFFDF6', borderLeftWidth: 1, borderTopWidth: 1, borderColor: GOLD,
    transform: [{ rotate: '45deg' }],
  },
  deepTipRow:  { flexDirection: 'row', alignItems: 'flex-start', gap: 9 },
  deepTipIcon: { width: 26, height: 26, borderRadius: 13, backgroundColor: GOLD + '1F',
                 alignItems: 'center', justifyContent: 'center' },
  deepTipTitle:{ fontSize: 13.5, fontWeight: '800', color: FOREST, marginBottom: 2 },
  deepTipCta:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
                 backgroundColor: FOREST, borderRadius: 10, paddingVertical: 9 },
  deepTipCtaText: { color: CREAM, fontWeight: '800', fontSize: 12.5, letterSpacing: 0.3 },

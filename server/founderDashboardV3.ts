...
const esc = (v: unknown): string =>
  ...
  function renderConversion(s: any): string {
    if (isErr(s)) return errorCard("Guest vs Account Conversion", s);
    const grid = [
      card("Guest sessions", num(s.guestSessions)),
      card("Onboarding starts", num(s.onboardingStarts)),
      card("Continue-as-guest taps", num(s.continueGuestTaps)),
    ...
    const note = s.guestToAccountTracked ? "" :
      `<div class="card"><div class="muted">Guest-to-account conversion not fully tracked yet (needs anon_id carried through signup).</div></div>`;
    return section("conversion", "4 · Guest vs Account Conversion", `<div class="stat-grid">${grid}</div>${note}`, "guests excluded from real users");
  }
  ...
  *{box-sizing:border-box}
  ...
  const TOC = [
    ...
    ["conversion", "Conversion"], ["sessions", "Sessions"], ["scans", "Scans"],

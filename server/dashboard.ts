...
export function generateDashboard({ entries, summary, scanStats, analytics, secret }: DashboardInput): string {
  ...
  return [
    ...
    "<div class='container'>",
    "<div class='exports' style='margin:16px 0;display:flex;align-items:center;flex-wrap:wrap;gap:8px'>",
    ...
    "<button onclick='openResetModal()' style='margin-left:auto;background:#4a1c1c;color:#fc8181;border:1.5px solid #fc818155;padding:8px 18px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;letter-spacing:0.5px'>⚠️ EMERGENCY RESET</button>",
    "</div>",

    // ── Reset modal ──────────────────────────────────────────────────────────
    "<div id='resetModal' style='display:none;position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:9999;align-items:center;justify-content:center'>",
    "  <div style='background:#1a2035;border:1.5px solid #fc818155;border-radius:14px;padding:32px;max-width:420px;width:90%;box-shadow:0 8px 40px #000'>",
    "    <h2 style='color:#fc8181;font-size:16px;margin-bottom:8px;font-weight:700'>⚠️ Emergency Analytics Reset</h2>",
    "    <p style='color:#a0aec0;font-size:13px;margin-bottom:20px;line-height:1.6'>This will permanently clear <strong style='color:#f0c040'>all</strong> analytics data — events, sessions, scan records, and feedback.<br><br>Type the passcode to confirm:</p>",
    "    <input id='resetPasscode' type='password' placeholder='Enter passcode' style='width:100%;background:#0f1117;color:#e2e8f0;border:1px solid #2d3748;border-radius:6px;padding:10px 12px;font-size:14px;margin-bottom:8px;box-sizing:border-box' onkeydown='if(event.key===\"Enter\")confirmReset()' />",
    "    <p id='resetError' style='color:#fc8181;font-size:12px;min-height:18px;margin-bottom:12px'></p>",
    "    <div style='display:flex;gap:10px'>",
    "      <button onclick='confirmReset()' style='flex:1;background:#7a1c1c;color:#fc8181;border:1.5px solid #fc818155;padding:10px;border-radius:6px;font-size:13px;font-weight:700;cursor:pointer'>CONFIRM RESET</button>",
    "      <button onclick='closeResetModal()' style='flex:1;background:#243050;color:#90cdf4;border:1px solid #2d3748;padding:10px;border-radius:6px;font-size:13px;cursor:pointer'>Cancel</button>",
    "    </div>",
    "  </div>",
    "</div>",
    ...
    "  setTimeout(()=>document.getElementById('resetPasscode').focus(),100);",
    "}",
    ...
    "  if(!passcode){errEl.textContent='Please enter the passcode.';return;}",
    "  const btn=document.querySelector('#resetModal button');",
    "  btn.textContent='Resetting...';btn.disabled=true;",
    "  try{",
    "    const res=await fetch('/api/dev/reset-analytics?secret=" + esc(secret) + "',{",
    "      method:'POST',",
    ...
    "    if(data.ok){",
    "      closeResetModal();",
    "      document.body.innerHTML='<div style=\"font-family:system-ui;background:#0f1117;color:#68d391;min-height:100vh;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px\"><div style=\"font-size:48px\">✅</div><div style=\"font-size:20px;font-weight:700\">Analytics Reset Complete</div><div style=\"font-size:14px;color:#a0aec0\">All data cleared. Dashboard starting fresh.</div><button onclick=\"location.reload()\" style=\"margin-top:16px;background:#1a2035;color:#90cdf4;border:1px solid #2d3748;padding:10px 24px;border-radius:6px;cursor:pointer;font-size:14px\">Reload Dashboard</button></div>';",
    "    } else {",
    "      errEl.textContent=data.error||'Incorrect passcode.';",
    "      btn.textContent='CONFIRM RESET';btn.disabled=false;",
    "    }",
    "  }catch(e){",

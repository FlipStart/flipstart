...
/**
 * Deletes EVERY file this user has in a bucket (their `{userId}/` folder).
 * Used by clear-all-history so wiping scans doesn't leave orphaned photos
 * consuming storage quota forever. Paginates because list() caps at 100.
 * Best-effort — never throws.
 */
export async function deleteAllUserImages(userId: string, bucket: UploadBucket): Promise<void> {
  try {
    const sb = await getSupabase();
    if (!sb) return;
    ...
    for (let page = 0; page < 50; page++) {
      ...
      if (error) {
        if (__DEV__) console.warn(`[imageUpload] list ${bucket}/${userId} failed:`, error.message);
        return;
      }
      ...
      if (rmErr) {
        if (__DEV__) console.warn(`[imageUpload] bulk remove failed:`, rmErr.message);
        return;
      }
      // Always re-list from offset 0 — removed files shift the window.
      if (data.length < 100) return;
    }
  } catch (err) {
    if (__DEV__) console.warn(`[imageUpload] deleteAllUserImages threw:`, err);
  }
}
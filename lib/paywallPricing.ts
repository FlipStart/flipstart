...
/**
 * The Annual plan expressed as a monthly figure: `priceAmount / 12`, formatted
 * in the ANNUAL product's own currency.
 *
 * This is the single most persuasive number on the paywall — "$3.33/month"
 * makes the comparison to "$7.99/month" concrete in a way "$39.99/year" never
 * will — and it is the easiest one to get wrong. So the rules are strict:
 *
 *   • derived from the live amount, never hardcoded
 *   • formatted with Intl in the product's currencyCode, so a Japanese
 *     storefront gets ¥ and no decimals, not "$3.33"
 *   • null when amount or currency is missing — a suppressed line is honest;
 *     an invented one is not
 *
 * Returns the bare formatted amount ("$3.33"). The caller appends "/month" so
 * the layout can style the two parts differently.
 */
export function annualMonthlyEquivalent(annual: ProductPricing): string | null {
  const a = annual.priceAmount;
  const cur = annual.currencyCode;
  if (a === null || !Number.isFinite(a) || a <= 0 || !cur) return null;
  const perMonth = a / 12;
  try {
    ...
    // An unrecognised currency code. Refuse rather than guess a symbol.
    return null;
  }
}
